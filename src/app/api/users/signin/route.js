...
console.log('token:', token);
    ...
    if (token) {
      ...
      const result = await pool.query('SELECT * FROM "User" WHERE user_id = $1', [decoded_token.user_id]);
    ...
    if (user) {
      return new Response(
        JSON.stringify({ success: true, message: "User is already logged in." }),
    ...
    const does_user_email_exist = await pool.query(
    ...
    if (does_user_email_exist.rows.length === 0) {
      return new Response(
        JSON.stringify({ success: false, message: "No user exists with given email" }),
        { status: 400 }
      );
    }
